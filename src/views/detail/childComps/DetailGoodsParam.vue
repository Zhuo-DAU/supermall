<template>
  <div class="goods-params" v-if="Object.keys(goodsParams).length">
    <div class="rules">
      <table v-if="goodsParams.sizes.length">
        <tr v-for="(tr, index) in goodsParams.sizes[0]" :key="index">
          <td v-for="(td, index) in tr" :key="index">{{td}}</td>
        </tr>
      </table>
    </div>
    <div class="params" v-if="goodsParams.infos.length">
      <table>
...
<style scoped>
  ...
  .rules table tr td, .params table tr td {
    padding: 8px 16px;
